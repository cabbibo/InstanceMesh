 ...
 // float w = 1. + snoise( pos * .01 );
  ...
  // The 'sub' objects
    ...
    if( mI.x < 5. ){
      ...
      vec3 attract = springForce( otherPos.xyz , pos.xyz , 3.4 );
      force += attract * 100.;
      ...
      for( int i = 0; i < 4; i++ ){
        ...
        if( (i - index) != 0 ){
          ...
          vec3 attract = springForce(  pos.xyz , otherPos.xyz , 2.8 );
    ...
    // The 'Sub Sub' objects
      ...
      vec3 attract = springForce( otherPos.xyz , pos.xyz , 3.3  );
    ...
    // Bundle around spine
      ...
      vec3 attract = springForce( otherPos.xyz , pos.xyz , 8.1  * uv.y  );
      ...
      for( int i = 0; i < 11; i++ ){
        ...
        if( i-index != 0 ){
          ...
          vec3 attract = springForce( pos.xyz , otherPos.xyz  , 20.1 * uv.y );
